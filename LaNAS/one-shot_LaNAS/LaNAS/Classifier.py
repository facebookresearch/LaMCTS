...
class Classifier():
    ...
    def get_params(self):
        ...
        return self.model.fc1.weight.detach().cpu().numpy(), self.model.fc1.bias.detach().cpu().numpy()
    ...
    def train(self):
        if self.training_counter == 0:
            self.epochs = 1000#20000
        else:
            self.epochs = 1000#3000
...
#
#print("total samples:", len(samples.keys() ))
#c = Classifier(samples, 10)
#goodies, badies = c.split_data()
#print("goodies:", len(goodies.keys() ), np.mean(np.array(list(goodies.values()) ) )," bad:", len(badies.keys() ), np.mean(np.array(list(badies.values()) )) )
